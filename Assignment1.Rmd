...
So, there might be the case that the data is obtained from same distribution but the plots are like this because of the difference of the location. Now, let us visualise the data after standardization of all the three clusters, namely, setosa, virginica and versicolor.
...
Clearly, after standardizing the data, all are clustered around the same point. Hence, we have done the DD-plots again after standardizing the data.
...
$$ q_{\mathbf{x}}(\mathbf{u}) = \text{argmin}_{\mathbf{q}} \left\{ E[|(\mathbf{X} - \mathbf{q})| - |(\mathbf{X})|] - \langle \mathbf{u}, \mathbf{q} \rangle \right\} $$

Here, $\langle \mathbf{u}, \mathbf{q} \rangle$ denotes the dot product between $\mathbf{u}$ and $\mathbf{q}$.
